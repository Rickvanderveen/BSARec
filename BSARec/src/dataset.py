...
class DataMaps:
    def __init__(
        ...
        self._id2user = id2user
        self._item2id = item2id
        self._id2item = id2item

    def user2id(self, user: str) -> Optional[str]:
        return self._user2id.get(user)

    def id2user(self, id: Union[str, int]) -> Optional[str]:
        return self._id2user.get(str(id))

    def item2id(self, item: str) -> Optional[str]:
        return self._item2id.get(item)

    def id2item(self, id: Union[str, int]) -> Optional[str]:
        return self._id2item.get(str(id))

    @classmethod
    def read_json(cls, path: Path):
        # Load the requested json file
        with path.open("r", newline="", encoding="utf-8") as file:
            datamaps = json.load(file)
        # Check if all datamaps exist in the json
        required_datamaps = ["user2id", "id2user", "item2id", "id2item"]
        for datamap in required_datamaps:
            if datamap not in datamaps:
                raise MissingDataMapError(datamap)

        return cls(**datamaps)


class MissingDataMapError(KeyError):
    """Raised when a required data map is missing from the input JSON."""

    def __init__(self, missing_map_name: str):
        super().__init__(
            f"Required data map '{missing_map_name}' is missing from JSON file"
        )
        self.missing_map_name = missing_map_name
